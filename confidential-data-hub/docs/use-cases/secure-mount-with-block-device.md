...
| `options.sourceType`      | `options`   | `"empty"` \| `"encrypted"`                       | Yes      | `"empty"` creates ephemeral detached-header storage and `"encrypted"` opens a caller-prepared device. The free-form API rejects `"persistent"`. |
...
| `options.key`             | `options`   | URI: `sealed.*`, `kbs://…`, or `file://…`         | No       | Where to fetch an encryption key. If omitted, CDH generates a random key. |
...
The free-form `secure_mount` API deliberately rejects `sourceType:
"persistent"`: it cannot carry the measured manifest and fixed crypto-profile
binding required before a destructive first-use operation. Confidential
persistent volumes use the Agent-only typed `SecureVolumeService` instead. See
[Confidential persistent block volumes](confidential-persistent-volume.md).
