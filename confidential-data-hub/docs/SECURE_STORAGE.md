...
- **Common request shape**: Both modes use `volume_type: "block-device"` and share the same basic options:
  ...
  - `sourceType`: `"empty"` (ephemeral detached-header provisioning) or `"encrypted"` (caller-prepared encrypted device).
...
- **LUKS2 mode**:
  ...
  - Confidential persistent storage does not use this free-form API. Kata Agent
    invokes CDH's typed `SecureVolumeService` over the owner-only guest Unix
    socket. The service binds a content-addressed manifest, exact device
    identity, fixed crypto profile, and access mode before activation. See
    [Confidential persistent block volumes](use-cases/confidential-persistent-volume.md).
- **ZFS mode**:
