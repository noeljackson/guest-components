...
mod tests {
    #[test]
    fn grpc_server_does_not_expose_secure_volume_activation() {
        let source = include_str!("mod.rs");
        assert!(!source.contains(concat!("SecureVolume", "ServiceServer")));
        assert!(!source.contains(concat!("impl SecureVolume", "Service for Cdh")));
    }
}
